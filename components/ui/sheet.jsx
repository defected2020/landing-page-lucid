...
const sheetVariants = cva(
  'fixed z-[1001] gap-4 bg-bg shadow-dropdown transition ease-smooth data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500',
  {
    variants: {
      side: {
  ...
  ({ side = 'right', className, children, hideClose = false, ...props }, ref) => (
    <SheetPortal>
      ...
      >
        {children}
        {!hideClose && (
          <DialogPrimitive.Close className="absolute right-4 top-4 rounded-full border border-border p-2 text-text-muted opacity-70 transition-all hover:opacity-100 hover:border-accent hover:bg-hover-overlay focus:outline-none focus:ring-2 focus:ring-accent disabled:pointer-events-none">
            <X className="h-4 w-4" />
